...
sudo perl -wpi -e 's!^#relay_log!relay_log!' /etc/mysql/mysql.conf.d/mysqld.cnf
...
SQL=$(cat <<EOF
change replication source to source_host = '${SOURCE_HOST}', source_port = ${SOURCE_PORT}, source_ssl = 1, source_auto_position = ${SOURCE_AUTO_POSITION}, source_ssl = ${SOURCE_SSL}, source_heartbeat_period = ${SOURCE_HEARTBEAT_PERIOD};
